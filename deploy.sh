...
REMOTE_JAR_FILE=gamedoora-project-services.jar
...
pwd
ls
sshpass -p $SSH_PASS scp -o StrictHostKeyChecking=no  ./gamedoora-project-services.service "$SSH_USERNAME@$SSH_HOST:$DEST_PATH"
echo "==========================="
echo "==Stopping service======="
# Stop the service
sshpass -p $SSH_PASS ssh -o StrictHostKeyChecking=no "$SSH_USERNAME@$SSH_HOST" 'sudo systemctl daemon-reload; sudo systemctl stop gamedoora-project-services'
echo "========================="
...
sshpass -p $SSH_PASS ssh "$SSH_USERNAME@$SSH_HOST" 'chown -R gamedoora:gamedoora /opt/gamedoora; chmod ug+rwx /opt/gamedoora/*.jar; systemctl restart gamedoora-project-services; systemctl is-active --quiet service && echo Service is running'
echo "========================="
